...
async def dashboard_data():
    ...
    try:
        ...
        kpis = run_query("SELECT SUM(opportunity) AS total_opportunity, SUM(rolling_12_sales) AS total_rolling_12_sales, COUNT(DISTINCT account) AS total_accounts, SUM(total_units_sold) AS total_units FROM __CATALOG__.__SCHEMA__.account_targeting")
        opp_by_product = run_query("SELECT product_line, SUM(opportunity) AS total_opportunity FROM __CATALOG__.__SCHEMA__.account_targeting GROUP BY product_line ORDER BY total_opportunity DESC")
        opp_by_target = run_query("SELECT target_type, SUM(opportunity) AS total_opportunity FROM __CATALOG__.__SCHEMA__.account_targeting GROUP BY target_type ORDER BY total_opportunity DESC")
        top_accounts = run_query("SELECT account, SUM(opportunity) AS total_opportunity, SUM(rolling_12_sales) AS total_rolling_12_sales, ROUND(AVG(penetration_2025)*100, 1) AS avg_penetration FROM __CATALOG__.__SCHEMA__.account_targeting GROUP BY account ORDER BY total_opportunity DESC LIMIT 10")
        opp_by_area = run_query("SELECT area, SUM(opportunity) AS total_opportunity FROM __CATALOG__.__SCHEMA__.account_targeting GROUP BY area ORDER BY total_opportunity DESC")
        vol_by_specialty = run_query("SELECT specialty, SUM(cy_procedure_volume) AS total_volume FROM __CATALOG__.__SCHEMA__.hcp_procedure_volume GROUP BY specialty ORDER BY total_volume DESC")
        top_surgeons = run_query("SELECT surgeon_name, cy_procedure_volume, specialty FROM __CATALOG__.__SCHEMA__.hcp_procedure_volume ORDER BY cy_procedure_volume DESC LIMIT 8")
